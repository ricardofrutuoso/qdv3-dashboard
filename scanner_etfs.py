# ============================================================
# SCANNER_ETFS.PY — ETFs Globais QDV3
# DFA Hurst | P/D VWAP | Brennan | Quad Framework
# ============================================================
...
def entry_quality(r):
    ...
    pd_1m= r.get("pd_1m", 0); h    = r.get("h_d", 0.5)
    vp   = r.get("vp", 50);   comp  = r.get("comp","NORMAL")
    score= r.get("score", 0); adx   = r.get("adx", 0)
    if (pd_c<0 and pd_1w<0 and pd_1m<0 and h>0.55 and
            vp<40 and adx>20 and "COMPRES" in comp and score<=-0.8):
        return "✅ IDEAL"
    if (pd_c<0 and pd_1w<0 and h>0.52 and vp<55 and score<=-0.6):
        return "🟡 ACEITÁVEL"
    if (pd_c>0 and pd_1w>0 and h>0.55 and vp<40 and "COMPRES" in comp):
        return "✅ IDEAL LONG"
    if pd_c > 0.05: return "⚠️ EXTENDED"
    if pd_c > 0.02 and score > 0.8: return "⏳ AGUARDA"
...
def run_etfs_scanner(limit=None):
    print(f"\n{'='*60}")
    print(f"  ETFs SCANNER — {RUN_DATE}")
    print(f"  DFA Hurst + P/D VWAP + Brennan Compression")
    print("="*60)
    tickers = list(set(ETFS))
    ...
    for symbol, name, market in tickers:
        try:
            ...
            r["entry_q"] = entry_quality(r)
            r["rank"]    = calc_rank(r)
            qty, notional, sl = position_size(
                r["price"], market, r["h_d"], r["vp"])
            r["qty"] = qty; r["notional"] = notional; r["sl"] = sl
            results.append(r)
...
def generate_etfs_html(results, top):
    def rows_html(lst):
        rows = ""
        for r in lst:
            eq = r.get("entry_q","—")
            ...
            <tr>
              ...
              <td style="color:#64748b">{r['name'][:20]}</td>
    ...
    header = """<tr>
        ...
        <th>Brennan</th><th>Z-TTM</th><th>Rank</th>
    </tr>"""
    return f"""
    <div id="etfs" class="tab-content section">
      <div class="section-title">ETFs GLOBAIS</div>
      ...
      <div style="color:#64748b;font-size:11px;font-weight:700;
                  margin-bottom:8px">🏆 TOP {TOP_N}</div>
      ...
      <div style="color:#64748b;font-size:11px;font-weight:700;
                  margin:16px 0 8px">🟡 ACEITÁVEIS</div>
      <div class="table-wrap">
