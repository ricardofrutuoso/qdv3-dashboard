# ============================================================
# SCANNER_COMMODITIES.PY — Commodities QDV3
...
def entry_quality(r, cot_signal=None):
    ...
    if cot_signal:
        ...
        cz   = cot_signal.get("commercial_z",0)
        if "BULLISH FORTE" in csig: cot_bonus = -0.6
        elif "BULLISH"     in csig: cot_bonus = -0.3
        elif "BEARISH FORTE" in csig: cot_bonus = 0.6
        elif "BEARISH"       in csig: cot_bonus = 0.3
    adj = score + cot_bonus
...
def calc_rank(r, cot_signal=None):
    ...
    if cot_signal:
        ...
        if "FORTE" in csig and cz>=2.0: score+=15
        elif csig not in ("","NEUTRO"): score+=8
        div  = cot_signal.get("divergence","")
        if div: score+=5
    return min(round(score),100)

def run_commodities_scanner(cot_data=None, limit=None):
    print(f"\n{'='*60}")
    ...
    for symbol, name, market in tickers:
        try:
            ...
            cot_str = ""
            if cot:
                cot_str = f" COT:{cot['cot_signal'][:8]} Z:{cot['commercial_z']:+.1f}"
...
def generate_commodities_html(results, top):
    def rows_html(lst):
        rows = ""
        for r in lst:
            eq  = r.get("entry_q","—")
            ...
            if cot:
                ...
                cz = cot.get("commercial_z",0)
                cot_cell = (
                    f"<span style='color:{cc};font-size:9px'>"
                    f"{cot['cot_signal'][:10]} Z:{cz:+.1f}</span>"
                )
            rows += f"""
            <tr>
    ...
    header = """<tr>
        ...
        <th>Entry Quality</th><th>Hurst</th>
        <th>P/D D</th><th>P/D W</th><th>P/D M</th>
        <th>Vol%</th><th>Z-TTM</th><th>COT</th><th>Rank</th>
    </tr>"""
    return f"""
